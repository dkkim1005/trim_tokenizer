"""
The original code is from https://github.com/1kkiRen/Tokenizer-Changer,
authored by Kuzmin Dmitrii.

Minor errors have been fixed to ensure the sentence piece tokenizer runs properly.
"""
...
class TokenizerChanger:
    def __init__(
        self, tokenizer: PreTrainedTokenizerFast = None, space_sign: str = "Ġ"
    ):
        ...
        self.state = (
            json.loads(tokenizer.backend_tokenizer.__getstate__()) if tokenizer else {}
        )
        if isinstance(self.state["model"]["vocab"], list):
            self.state["model"]["vocab"] = dict(self.state["model"]["vocab"])
        self.initial_length = len(self.state["model"]["vocab"]) if self.state else 0
    ...
    def _fill_unwanted_merges(self, batch: list[str]):
        ...
        self.__is_tokenizer()
        unwanted_merges = []
        for processed_merge, original_merge in tqdm(
            batch, desc="Finding unwanted merges"
        ):
    ...
    def format_merges(self):
        ...
        for i in tqdm(
            range(len(self.state["model"]["merges"])), desc="Formatting merges"
        ):
    ...
    def _move_special_tokens(self):
        ...
        for i in tqdm(
            range(len(self.state["added_tokens"])), desc="Moving special tokens"
        ):
        ...
        for i in range(len(self.state["post_processor"]["processors"])):
            if "special_tokens" in self.state["post_processor"]["processors"][i].keys():
                for k in self.state["post_processor"]["processors"][i][
                    "special_tokens"
                ].keys():
                    for j in tqdm(
                        range(
                            len(
                                self.state["post_processor"]["processors"][i][
                                    "special_tokens"
                                ][k]["ids"]
                            )
                        ),
                        desc="Moving special tokens",
                    ):
                        self.state["post_processor"]["processors"][i]["special_tokens"][
                            k
                        ]["ids"][j] += (
                            len(self.state["model"]["vocab"]) - self.initial_length
                        )
    ...
    def find_least_tokens(
        self,
        k_least: int,
        exclude: list[str] = [],
        consider_excluded_tokens: bool = False,
    ):
        ...
        for k, v in tqdm(
            dict(reversed(list(self.state["model"]["vocab"].items()))).items(),
            desc="Finding unwanted tokens",
        ):
    ...
    def find_token_id_gap(self):
        ...
        reversed_vocab_values = list(reversed(self.state["model"]["vocab"].values()))
        last_gap = 0
    ...
    def add_merges(self, merges: list[str]):
        ...
        processed_merges = [("".join(merge), merge) for merge in merges]

        vocab = set(self.state["model"]["vocab"].keys())

        for processed_merge, merge in tqdm(
            iterable=processed_merges, desc="Adding merges"
        ):
    ...
    def _simple_delete_merges(self, processed_merges: list[str]):
        ...
        for processed_merge, original_merge in tqdm(
            processed_merges, desc="Finding unwanted merges"
        ):
            if any(token in processed_merge for token in self.unwanted_tokens):
                unwanted_merges_set.add(tuple(original_merge))

        self.state["model"]["merges"] = [
            merge
            for merge in tqdm(
                self.state["model"]["merges"], desc="Deleting unwanted merges"
            )
            if tuple(merge) not in unwanted_merges_set
        ]
    ...
    def delete_merges(self, unwanted_tokens: list[str] = None, n_jobs=1):
        ...
        processed_merges = [
            ("".join(merge), merge) for merge in self.state["model"]["merges"]
        ]
        unwanted_merges_set = set()

        self.unwanted_tokens = (
            list(set(unwanted_tokens))
            if unwanted_tokens
            else list(set(self.unwanted_tokens))
        )
        ...
        if n_jobs > 1:
            try:
                ...
                chunks = [
                    processed_merges[i : i + chunk_size]
                    for i in range(0, len(processed_merges), chunk_size)
                ]
                with Pool(n_jobs) as pool:
                    unwanted_merges_batches = list(
                        tqdm(
                            pool.map(self._fill_unwanted_merges, chunks),
                            total=len(chunks),
                            desc="Processing merges",
                        )
                    )

                unwanted_merges = list(
                    merge
                    for merge_batch in unwanted_merges_batches
                    for merge in merge_batch
                )
                unwanted_merges_set = set()
                for unwanted_merge in tqdm(
                    unwanted_merges, desc="Filling unwanted merges"
                ):
    ...
    def delete_inappropriate_merges(self, vocab: list[str]):
        ...
        for processed_merge, original_merge in tqdm(
            ...
            if not all(
                token in vocab
                for token in [processed_merge, original_merge[0], original_merge[1]]
            ):
        ...
        self.state["model"]["merges"] = [
            ...
            if merge not in unwanted_merges_set
        ]
    ...
    def delete_tokens(
        ...
        for token in tqdm(
            list(set(self.unwanted_tokens)), desc="Deleting unwanted words"
        ):
            try:
    ...
    def delete_k_least_frequent_tokens(
        self,
        k: int,
        exclude: list[str] = [],
        delete_merges: bool = True,
        ...
        Args:
            ...
            n_jobs (int, optional): number of threads while deleting merges. Defaults to 1.
        """
        self.find_least_tokens(
            k_least=k,
            exclude=exclude,
            consider_excluded_tokens=consider_excluded_tokens,
        )
        self.delete_tokens(
            include_substrings=False, delete_merges=delete_merges, n_jobs=n_jobs
        )
    ...
    def get_overlapping_merges(self, merges: list):
        ...
        processed_merges_new_tokenizer = [
            ("".join(merge).replace(" ", ""), merge)
            for merge in self.state["model"]["merges"]
        ]

        processed_merges_old_tokenizer = [
            ("".join(merge).replace(" ", ""), merge) for merge in merges
        ]

        for merge in tqdm(
            processed_merges_new_tokenizer, desc="Finding overlapping merges"
        ):
            if any(
                merge in processed_merge
                for processed_merge in processed_merges_old_tokenizer
            ):
    ...
    def updated_tokenizer(self):
        ...
        if isinstance(self.state["model"]["vocab"], dict):
            self.state["model"]["vocab"] = [
                (k, v) for k, v in self.state["model"]["vocab"].items()
            ]
        ...
        self.state = json.loads(self.tokenizer.backend_tokenizer.__getstate__())
